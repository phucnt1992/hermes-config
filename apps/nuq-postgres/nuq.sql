...
-- Cluster-level PostgreSQL tuning is intentionally not applied here.
-- Keep schema/bootstrap migrations re-runnable under least-privilege roles.
-- Apply system tuning separately using apps/nuq-postgres/postgresql18-tuning.sql.

CREATE SCHEMA IF NOT EXISTS nuq;

-- Idempotent pg_cron registration helper for re-runnable deployments.
CREATE OR REPLACE FUNCTION nuq.schedule_job(p_jobname text, p_schedule text, p_command text)
RETURNS bigint
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  RETURN cron.schedule(p_jobname, p_schedule, p_command);
END;
$$;
...
SELECT nuq.schedule_job('nuq_queue_scrape_clean_completed', '*/5 * * * *', $$
  ...
  DELETE FROM nuq.queue_scrape q
  USING doomed d
  WHERE q.ctid = d.ctid;
$$);

SELECT nuq.schedule_job('nuq_queue_scrape_clean_failed', '*/5 * * * *', $$
  SET statement_timeout = '30s';
  WITH doomed AS (
    SELECT ctid
    FROM nuq.queue_scrape
...
SELECT nuq.schedule_job('nuq_queue_scrape_lock_reaper', '15 seconds', $$
  SET statement_timeout = '30s';
...
SELECT nuq.schedule_job('nuq_queue_scrape_backlog_reaper', '* * * * *', $$
  SET statement_timeout = '50s';
  WITH doomed AS (
    SELECT ctid
    FROM nuq.queue_scrape_backlog
    WHERE nuq.queue_scrape_backlog.times_out_at < now()
    ORDER BY times_out_at
    LIMIT 10000
  )
  DELETE FROM nuq.queue_scrape_backlog b
  USING doomed d
  WHERE b.ctid = d.ctid;
$$);

-- Per-index REINDEX CONCURRENTLY, spread across the weekly low-traffic window
-- (Sunday 02:00-10:00 UTC). REINDEX INDEX takes a ShareUpdateExclusiveLock only on
...
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_pkey',                   '0 2 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.queue_scrape_pkey;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_active_locked_at',       '20 2 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.queue_scrape_active_locked_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_queued_optimal_2',       '40 2 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_queued_optimal_2_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_failed_created_at',      '0 3 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_failed_created_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_group_owner_mode',       '20 3 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_group_owner_mode_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_group_mode_status',      '40 3 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_group_mode_status_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_group_completed_listing','0 4 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_group_completed_listing_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_group_status',           '20 4 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.idx_queue_scrape_group_status;$$);

SELECT nuq.schedule_job('nuq_reindex_queue_scrape_backlog_pkey',           '40 4 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.queue_scrape_backlog_pkey;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_backlog_owner_id',       '0 5 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_backlog_owner_id_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_backlog_group_mode',     '20 5 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_backlog_group_mode_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_backlog_group_id',       '40 5 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.idx_queue_scrape_backlog_group_id;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_backlog_times_out_at',   '0 6 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_backlog_times_out_at_idx;$$);

SELECT nuq.schedule_job('nuq_reindex_queue_scrape_completed_standalone',   '20 6 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_completed_standalone_created_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_failed_standalone',      '40 6 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_failed_standalone_created_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_scrape_group_id',               '0 7 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_scrape_group_id_idx;$$);
...
SELECT nuq.schedule_job('nuq_maintenance_watchdog', '* * * * *', $$
...
SELECT nuq.schedule_job('nuq_queue_crawl_finished_clean_completed', '*/5 * * * *', $$
  SET statement_timeout = '30s';
  WITH doomed AS (
    SELECT ctid
    FROM nuq.queue_crawl_finished
  ...
  DELETE FROM nuq.queue_crawl_finished q
  USING doomed d
  WHERE q.ctid = d.ctid;
$$);

SELECT nuq.schedule_job('nuq_queue_crawl_finished_clean_failed', '*/5 * * * *', $$
  SET statement_timeout = '30s';
  WITH doomed AS (
    SELECT ctid
    FROM nuq.queue_crawl_finished
...
SELECT nuq.schedule_job('nuq_queue_crawl_finished_lock_reaper', '15 seconds', $$
  SET statement_timeout = '30s';
...
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_pkey',                 '20 7 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.queue_crawl_finished_pkey;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_active_locked_at',     '40 7 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.queue_crawl_finished_active_locked_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_queued_optimal_2',     '0 8 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_crawl_finished_queued_optimal_2_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_failed_created_at',    '20 8 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_crawl_finished_failed_created_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_completed_created_at', '40 8 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_crawl_finished_completed_created_at_idx;$$);

SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_completed_standalone', '0 9 * * 0',  $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_crawl_finished_completed_standalone_created_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_failed_standalone',    '20 9 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_crawl_finished_failed_standalone_created_at_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_queue_crawl_finished_group_id',             '40 9 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_queue_crawl_finished_group_id_idx;$$);
SELECT nuq.schedule_job('nuq_reindex_group_crawl_completed_expires_at',          '0 10 * * 0', $$REINDEX INDEX CONCURRENTLY nuq.nuq_group_crawl_completed_expires_at_idx;$$);
...
SELECT nuq.schedule_job('nuq_group_crawl_finished', '15 seconds', $$
  SET statement_timeout = '30s';
...
SELECT nuq.schedule_job('nuq_group_crawl_clean', '* * * * *', $$
...
SELECT nuq.schedule_job('cron_job_run_details_prune', '0 * * * *', $$
  SET statement_timeout = '30s';
  WITH doomed AS (
    SELECT ctid
